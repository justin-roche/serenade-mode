...
(defun serenade--handle-command (command message callback) 
  ...
  (let* ((type (ht-get* command "type")) 
    ...
    (condition-case err (cond ((equal type "COMMAND_TYPE_GET_EDITOR_STATE") 
                               (progn ;;
                                 (let* ((results (funcall
                              ...
                              ((equal type "COMMAND_TYPE_DIFF") 
                               (progn (funcall (serenade-mode-configuration-diff
                                                serenade-active-mode-configuration)
                                               (ht-get command "source") 
                                               (+ 1 (or (ht-get command "cursor") 
                                                        0))))) 
                              ...
                              ((cond ((equal type "COMMAND_TYPE_EVALUATE_IN_PLUGIN") 
                                     ...
                                     ((equal type "COMMAND_TYPE_COPY") 
                                      (serenade--execute-builtin-command "copy <target>" (list
                                                                                          (ht-get*
                                                                                           command
                                                                                           "text"))))
                                     ((equal type "COMMAND_TYPE_OPEN_FILE_LIST") 
                                      (serenade--execute-builtin-command "open <file>" (list
                                                                                        (ht-get*
                                                                                         command
                                                                                         "path"))))
                                     ((equal type "COMMAND_TYPE_SWITCH_TAB") 
                                      (serenade--execute-builtin-command "<nth> tab" (list (ht-get*
                                                                                            command
                                                                                            "index"))))
                                     ((equal type "COMMAND_TYPE_SELECT") 
                                      (serenade--execute-builtin-command "select <target>" (list (+
                                                                                                  1
                                                                                                  (or
                                                                                                   (ht-get*
                                                                                                    command
                                                                                                 ...
                                                                                                 (+
                                                                                                  ...
                                                                                                  (ht-get*
                                                                                                   command
                                                                                                   "cursorEnd")))))
                                     (t (serenade--execute-builtin-command (ht-get* message "data"
                                                                                    "response"
                                                                                    "execute"
                                                                                    "transcript"))))))
      (debug))))
